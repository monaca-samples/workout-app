...
const Profile = ({ drawerAnchor, toggleDrawer, changeTheme }) => {
  ...
  const bmiExplanation = (bmi) => {
    if (Number(bmi) < 18.5) {
      return {
        type: "Underweight",
        message:
          "You are currently underweight, and should try to gain weight," +
      ...
      return {
        type: "Overweight",
        message:
          "Your current weight is over the normal weight. First consider " +
      ...
      return {
        type: "Obese",
        message:
          "Your current weight is over the normal weight." +
          " There are many risks associated with being overweight, such as high blood" +
          " pressure, heart disease, diabetes, etc., and an increased risk of mortality." +
  ...
  const handleSaveFitness = () => {
    ...
    setUser({
      ...user,
      height: cHeight,
      weights: [...user.weights, { date: formattedToday, weight: cWeight }],
    });
    updateDoc(docRef, {
      height: Number(cHeight),
      weights: [...user.weights, { date: formattedToday, weight: cWeight }],
    });
  ...
  return (
    <Box>
      ...
      <Container sx={{ pt: 10 }} maxWidth="lg">
        ...
        <Box sx={{ mb: 2 }}>
          <Typography variant="h6">Fitness information</Typography>
          <Paper
            sx={{
              display: "flex",
              flexDirection: "column",
